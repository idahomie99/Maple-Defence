// 🔥 1.0.41 버전 - 삭제되었던 인게임 draw() 함수 복구 및 런타임 에러 완벽 해결
...
function calculateEquipStats() {
    ...
    ['뱃지', '엠블럼', '링'].forEach(slot => {
        ...
        if (item) { equipStats.atk += item.atk || 0; equipStats.spd += item.spd || 0; equipStats.crit += item.crit || 0; }
    });
}
...
window.syncToCloud = async () => {
    ...
    let cloudProfile = {
        ...
        rp: userRankData.rp, rankMoney: userRankData.rankMoney, bonusCoins: userRankData.bonusCoins,
        raidDate: localStorage.getItem('mapleDefenseRaidDate') || null,
        inventory: userInventory, equips: userEquips, equipped: userEquipped
    };
...
window.closeAllModals = () => {
    ...
    ['overlay', 'bulk-sell-modal', 'ticket-modal', 'book-modal', 'shop-modal', 'active-skills-modal'].forEach(id => {
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (nickSnap.exists()) {
            ...
            const changeSnap = await get(child(dbRef, `users/${currentUserUid}/lastNicknameChange`));
            if (changeSnap.exists()) lastNicknameChange = changeSnap.val();
        } else {
            document.getElementById('nickname-overlay').style.display = 'block'; document.getElementById('nickname-modal').style.display = 'block';
        }
...
window.submitNickname = async () => {
    ...
    try {
        const now = Date.now();
        await update(ref(database, `users/${currentUserUid}`), { nickname: input, lastNicknameChange: now });
        currentUserName = input; lastNicknameChange = now;
        document.getElementById('current-user-name').innerText = currentUserName;
        document.getElementById('nickname-overlay').style.display = 'none'; document.getElementById('nickname-modal').style.display = 'none';
        window.switchScreen('start-screen');
    } catch (e) { showMessage("닉네임 저장 중 오류가 발생했습니다."); }
};

window.openNicknameChangeModal = () => {
    if (!currentUserUid) return;
    const now = Date.now(); const daysSinceLastChange = (now - lastNicknameChange) / (1000 * 60 * 60 * 24);
    if (lastNicknameChange > 0 && daysSinceLastChange < 30) {
        const remainingDays = Math.ceil(30 - daysSinceLastChange);
        showMessage(`닉네임은 30일에 한 번만 변경 가능합니다. (${remainingDays}일 남음)`); return;
    }
...
window.submitNicknameChange = async () => {
    ...
    try {
        const now = Date.now();
        await update(ref(database, `users/${currentUserUid}`), { nickname: input, lastNicknameChange: now });
        update(ref(database, `pk_rankings/${currentUserUid}`), { nickname: input }).catch(e => {});
        currentUserName = input; lastNicknameChange = now;
        document.getElementById('current-user-name').innerText = currentUserName;
        window.closeNicknameChangeModal(); showMessage("닉네임이 성공적으로 변경되었습니다!");
    } catch (e) { showMessage("닉네임 변경 중 오류가 발생했습니다."); }
};
...
// 4. 게임 데이터 (도감, 스킬, 상점)
// ==========================================
let bestWaveStr = localStorage.getItem('mapleDefenseBestWave'); let bestWave = bestWaveStr ? parseInt(bestWaveStr) : 0; if (isNaN(bestWave)) bestWave = 0; document.getElementById('ui-best-wave').innerText = bestWave;
let cardData = JSON.parse(localStorage.getItem('mapleDefenseCards')) || {}; const CARD_REQ = [1, 2, 4, 8, 12, 16, 20, 24, 28, 32]; 
...
function getBossInfo(w) {
    ...
    if (w >= 100 && w % 5 === 0) {
        let n = (w - 100) / 5; let calculatedHp = 2000000 + (n * 1000000) + (Math.pow(n, 2) * 150000); let bName = BOSS_WAVES[w] ? BOSS_WAVES[w].name : (w % 10 === 5 ? "어둠의 늑대" : `심연의 보스 (${w}층)`); let bMeso = BOSS_WAVES[w] ? BOSS_WAVES[w].meso : 150; let bTicket = BOSS_WAVES[w] ? BOSS_WAVES[w].ticket : 5; return { hp: Math.floor(calculatedHp), meso: bMeso, ticket: bTicket, name: bName };
    }
    return null;
}
...
window.goToLobby = () => { 
    ...
    window.closeAllModals(); document.getElementById('ui-best-wave').innerText = bestWave; window.switchScreen('start-screen');
    if (currentUserUid) window.syncToCloud();
};
...
window.summonUnit = () => {
    ...
    if(state.meso < 10) { showMessage("메소가 부족합니다!"); return; }
...
window.openTicketModal = () => {
    ...
    let currentTicketTier = state.tickets[0]; document.getElementById('ticket-tier').innerText = currentTicketTier; document.getElementById('overlay').style.display = 'block'; document.getElementById('ticket-modal').style.display = 'block'; updateUI();
};
window.useTicket = (choice) => {
    let emptyIdx = grid.findIndex(v => v === null); if(emptyIdx === -1) { showMessage("공간 부족!"); return; }
    let tier = state.tickets.shift(); let cls = choice === '랜덤' ? Object.keys(CLASSES)[Math.floor(Math.random()*3)] : choice; if(choice === '랜덤' && Math.random() < 0.2) tier++;
...
function draw() {
    if (!ctx) return; 
    ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = "rgba(188, 170, 164, 0.2)"; ctx.lineWidth = 35; ctx.lineJoin = "round"; ctx.beginPath(); 
    if (state.isRank) { ctx.rect(25, 25, 450, 240); } else { ctx.rect(25, 25, 450, 450); } 
    ctx.stroke();
...
function renderEquippedSlots() {
    ['뱃지', '엠블럼', '링'].forEach(slot => { let el = document.getElementById(`slot-${slot}`); let item = userEquipped[slot]; if (item) { el.className = `equip-slot equip-${item.grade.toLowerCase()}`; el.querySelector('.slot-item').innerHTML = getEquipIcon(slot); } else { el.className = `equip-slot`; el.querySelector('.slot-item').innerHTML = ''; } });
...
window.combineCoinPieces = () => {
    ...
    if (userInventory.coinPieces >= 10) {
        if (confirm("코인 조각 10개를 스킬 코인 1개로 합치시겠습니까?")) {
            window.isCombiningCoin = true; userInventory.coinPieces -= 10; userRankData.bonusCoins += 1;
            window.syncToCloud().then(() => { window.isCombiningCoin = false; renderInventoryTab('consumable'); showMessage("코인 1개를 획득했습니다!"); });
        }
...
window.openBox = (boxType) => {
    ...
    if (boxType === 'equipBoxes') { if (userInventory.equipBoxes <= 0) return; userInventory.equipBoxes--; generateEquipment(); } 
        ...
        userInventory.coinPieces += getFrag; let getEquipBox = Math.random() < data.equip ? 1 : 0; userInventory.equipBoxes += getEquipBox;
        showMessage(`${boxType} 개봉!\n조각 +${getFrag}, 장비상자 +${getEquipBox}`);
    }
...
window.startRankMatchmaking = async () => {
    ...
    if (playCount >= 10) { showMessage("오늘의 랭크 게임 제한 횟수를 모두 소진했습니다!"); return; }
...
window.startPkGame = (clsName) => {
    ...
    let pkBarContainer = document.getElementById('pk-global-bar-container');
    if (pkBarContainer) { if ((cls.type === '전사' && skillLevels.war_death > 0) || (cls.type === '법사' && skillLevels.mage_thunder > 0) || (cls.type === '도적' && skillLevels.thief_fuma > 0)) { pkBarContainer.style.display = 'block'; document.getElementById('pk-global-bar').style.background = cls.type === '전사' ? '#ffeb3b' : (cls.type === '법사' ? '#00e5ff' : '#ab47bc'); document.getElementById('pk-global-bar').style.width = '0%'; } else pkBarContainer.style.display = 'none'; }
    
    pkState = { active: true, time: 60, score: 0, lastTime: performance.now(), speed: 1, bestScore: 0, unit: { cls: cls, grade: grade, gradeIdx: 8, x: 110, y: 250, lastAttack: 0, globalCooldown: 0 }, scarecrow: { x: 390, y: 250, size: 20, freezeTimer: 0, freezeTickTimer: 0, freezeDmgVal: 0 }, projectiles: [], dmgTexts: [], vfx: [] };
    document.getElementById('pk-score').innerText = '0'; document.getElementById('pk-time').innerText = '60'; document.getElementById('pk-btn-speed').innerText = "1배속"; 
    
    if (currentUserUid) { get(child(ref(database), `pk_rankings/${currentUserUid}`)).then(snap => { if(snap.exists() && typeof snap.val().score === 'number') pkState.bestScore = snap.val().score; }).catch(e => {}); }
    pkLoop();
};

function pkLoop() { 
    if (!pkState.active) return; let now = performance.now(); 
    if (!pkState.lastTime) pkState.lastTime = now;
    let dtReal = (now - pkState.lastTime) / 1000; if (dtReal > 0.1) dtReal = 0.1; if (dtReal < 0) dtReal = 0.016;
    let dt = dtReal * (pkState.speed || 1); pkState.lastTime = now;

    pkState.time -= dtReal; document.getElementById('pk-time').innerText = Math.ceil(Math.max(0, pkState.time));
    if (pkState.time <= 0) {
...
window.submitPkScore = async () => {
    ...
    if (!currentUserUid) { showMessage("로그인이 끊어졌습니다."); window.switchScreen('start-screen'); return; }
    ...
    try {
        ...
        if (snap.exists()) { const data = snap.val(); if (typeof data.score === 'undefined' || finalScore > data.score) { shouldUpdate = true; } } else { shouldUpdate = true; }
        if (shouldUpdate) { await set(ref(database, `pk_rankings/${currentUserUid}`), { nickname: currentUserName, class: className, score: finalScore, date: new Date().toLocaleDateString() }); }
    } catch(e) { showMessage("서버 통신 오류 발생"); }
...
function drawPk() {
    ...
    if (husooabiImg && husooabiImg.complete && husooabiImg.naturalWidth > 0) { pkCtx.save(); pkCtx.translate(m.x, m.y); if (m.freezeTimer > 0) { pkCtx.globalAlpha = 0.5; pkCtx.fillStyle = "#81d4fa"; pkCtx.fillRect(-size * 1.5, -size * 1.5, size * 3, size * 3); pkCtx.globalAlpha = 1.0; } pkCtx.drawImage(husooabiImg, -size * 1.5, -size * 1.5, size * 3, size * 3); pkCtx.restore(); } else { pkCtx.font = "40px NanumSquare"; pkCtx.textAlign = "center"; pkCtx.textBaseline = "middle"; pkCtx.fillText("🎃", m.x, m.y);  }

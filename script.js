...
import { getDatabase, ref, set, get, child, onValue, remove, onDisconnect, runTransaction } from "https://www.gstatic.com/firebasejs/12.16.0/firebase-database.js";
...
let raidState = {
    ...
    initialKillCount: -1, rewardClaimedForKills: [],
...
window.switchScreen = (screenId) => {
    const screens = ['login-screen', 'start-screen', 'game-container', 'pk-game', 'raid-game'];
    screens.forEach(id => {
...
projImages.warrior1.src = "image/warrior1.png"; projImages.warrior2.src = "image/warrior2.png";
projImages.mage1.src = "image/magician1.png"; projImages.mage2.src = "image/magician2.png";
projImages.rogue1.src = "image/rogue1.png"; projImages.rogue2.src = "image/rogue2.png";
...
function setGridMode(mode) {
    ...
    let gridSize = isRankGrid ? 10 : 25;
    
    canvas.height = canvasHeight;
    document.getElementById('board-area').style.aspectRatio = isRankGrid ? "500/290" : "1/1";
    
    gridContainer.style.gridTemplateRows = isRankGrid ? "repeat(2, 1fr)" : "repeat(5, 1fr)";
...
window.startNewGame = () => {
    ...
    state = {
        ...
        upgrades: { '전사': {val: 0, cost: 10}, '법사': {val: 0, cost: 10}, '도적': {val: 0, cost: 10} }, tickets: [], isRank: false
    };
    
    document.getElementById('btn-speed').innerText = "1배속"; document.getElementById('btn-speed').style.display = 'block'; document.getElementById('btn-exit').style.display = 'block';
    let surrenderBtn = document.getElementById('btn-rank-surrender'); if (surrenderBtn) surrenderBtn.style.display = 'none';
...
window.loadAndStartGame = () => {
    ...
    state.isRank = false; state.wave = saved.wave; state.meso = saved.meso; state.mp = saved.mp; state.mpTotal = saved.mpTotal; state.kills = saved.kills; state.upgrades = saved.upgrades; state.tickets = saved.tickets;
    state.speed = 1;
    document.getElementById('btn-speed').innerText = "1배속"; document.getElementById('btn-speed').style.display = 'block'; document.getElementById('btn-exit').style.display = 'block';
    let surrenderBtn = document.getElementById('btn-rank-surrender'); if (surrenderBtn) surrenderBtn.style.display = 'none';
    document.getElementById('opp-board-wrapper').style.display = 'none';
    ...
    towers = [];
    if(saved.gridData && Array.isArray(saved.gridData)) { saved.gridData.forEach((u) => { if(u) window.addUnit(u.idx, u.gradeIdx, u.clsName, true); }); }
...
window.goToLobby = () => { 
    ...
    state.status = 'TITLE'; cancelAnimationFrame(mainReqId);
    let gameOverModal = document.getElementById('gameover-modal'); if(gameOverModal) gameOverModal.style.display = 'none';
    let overlay = document.getElementById('overlay'); if(overlay) overlay.style.display = 'none';
    window.closeAllModals(); document.getElementById('best-record').innerText = bestWave; window.switchScreen('start-screen');
    if (currentUserUid) window.syncToCloud();
};
...
window.upgradeCard = (bName) => {
    let data = cardData[bName]; let req = CARD_REQ[data.grade];
    if(data.grade < 10 && data.owned >= req) { data.owned -= req; data.grade++; localStorage.setItem('mapleDefenseCards', JSON.stringify(cardData)); if (currentUserUid) window.syncToCloud(); window.renderBook(); }
};
...
window.closeAllModals = () => {
    ...
    document.getElementById('overlay').style.display = 'none'; document.getElementById('bulk-sell-modal').style.display = 'none'; document.getElementById('ticket-modal').style.display = 'none'; document.getElementById('book-modal').style.display = 'none'; document.getElementById('shop-modal').style.display = 'none'; document.getElementById('active-skills-modal').style.display = 'none';
};
...
window.summonUnit = () => {
    ...
    if(state.meso < 10) return; let emptyIdx = grid.findIndex(v => v === null);
...
function showSummonToast(gradeName, gradeIdx, clsName, color) {
    let toast = document.getElementById('summon-toast'); let fontSize = 16 + (gradeIdx * 2); 
    toast.innerHTML = `<span style="color:${color}">${gradeName}</span> ${clsName}!`; toast.style.fontSize = fontSize + 'px'; toast.style.color = '#fff'; toast.className = 'toast-show';
    setTimeout(() => { toast.className = ''; }, 1500);
    if (gradeIdx >= 5) { let container = document.getElementById('game-container'); container.classList.add('shake-active'); setTimeout(() => container.classList.remove('shake-active'), 400); }
}

function showBossToast(name, isDrop = false) {
    let toast = document.getElementById('boss-toast');
    if (isDrop) { toast.innerHTML = `🃏 ${name} 카드 획득! 🃏`; toast.style.color = '#ffca28'; } else { toast.innerHTML = `⚠️ 보스 출현: ${name} ⚠️`; toast.style.color = '#ff5252'; }
    toast.className = 'toast-show'; setTimeout(() => { toast.className = ''; }, 2500);
}

window.addUnit = (idx, gradeIdx, clsName, isLoad = false) => {
    let grade = GRADES[gradeIdx]; let cls = CLASSES[clsName];
    let unit = { idx: idx, gradeIdx: gradeIdx, grade: grade, cls: cls, x: 75 + (idx % 5) * 70 + 35, y: 75 + Math.floor(idx / 5) * 70 + 35, lastAttack: 0, bindCooldown: 0, globalCooldown: 0 };
    grid[idx] = unit; towers.push(unit); if(!isLoad) showSummonToast(grade.name, gradeIdx, clsName, cls.color); renderGrid();
};
...
window.onCellClick = (idx) => {
    ...
    if (selectedUnitIdx !== -1) {
        if (selectedUnitIdx === idx) { selectedUnitIdx = -1; } 
            ...
            let target = grid[idx]; grid[idx] = grid[selectedUnitIdx];
            if (grid[idx]) { grid[idx].idx = idx; grid[idx].x = 75 + (idx % 5) * 70 + 35; grid[idx].y = 75 + Math.floor(idx / 5) * 70 + 35; }
...
window.sellSelectedUnit = () => { if(selectedUnitIdx === -1) return; let u = grid[selectedUnitIdx]; if(u && u.grade.sell > 0) { state.meso += u.grade.sell; towers = towers.filter(t => t !== u); grid[selectedUnitIdx] = null; selectedUnitIdx = -1; renderGrid(); updateUI(); } };
...
window.executeBulkSell = (type, value) => {
    ...
    for(let i = 0; i < grid.length; i++) {
        let u = grid[i]; if(!u || u.grade.sell === 0) continue; let match = false;
        if(type === 'class' && u.cls.type === value) match = true; if(type === 'grade' && u.gradeIdx <= value) match = true;
        if(match) { earnedMeso += u.grade.sell; towers = towers.filter(t => t !== u); grid[i] = null; soldCount++; }
    }
    if(soldCount > 0) { state.meso += earnedMeso; showMessage(`${soldCount} 유닛 판매 (+${earnedMeso} 메소)`); selectedUnitIdx = -1; renderGrid(); updateUI(); } else { showMessage("조건에 맞는 유닛이 없습니다."); }
    window.closeAllModals();
};
...
function renderGrid() {
    ...
    for(let i=0; i<grid.length; i++) {
        ...
        if(u) {
            if (u.gradeIdx === 6) cells[i].classList.add('glow-6'); if (u.gradeIdx === 7) cells[i].classList.add('glow-7'); if (u.gradeIdx === 8) cells[i].classList.add('glow-8');
            let barsHtml = '';
    ...
    if(pkBarContainer && typeof pkState !== 'undefined' && pkState.active && pkState.unit) {
        let u = pkState.unit;
        if ((u.cls.type === '전사' && skillLevels.war_death > 0) || (u.cls.type === '법사' && skillLevels.mage_thunder > 0) || (u.cls.type === '도적' && skillLevels.thief_fuma > 0)) {
            pkBarContainer.style.display = 'block'; let color = u.cls.type === '전사' ? '#ffeb3b' : (u.cls.type === '법사' ? '#00e5ff' : '#ab47bc'); document.getElementById('pk-global-bar').style.background = color;
        }
    }
}

function renderOppGrid() {
    let oppGridContainer = document.getElementById('opp-grid-container'); if(!oppGridContainer) return;
    let cells = oppGridContainer.children; if(!cells || cells.length === 0) return;
    for(let i=0; i<oppGrid.length; i++) {
        let u = oppGrid[i]; cells[i].className = 'grid-cell';
        if(u) {
            if (u.gradeIdx === 6) cells[i].classList.add('glow-6'); if (u.gradeIdx === 7) cells[i].classList.add('glow-7'); if (u.gradeIdx === 8) cells[i].classList.add('glow-8');
...
function nextWave() {
    ...
    if (!state.isRank && state.wave > bestWave) { bestWave = state.wave; localStorage.setItem('mapleDefenseBestWave', bestWave); document.getElementById('best-record').innerText = bestWave; if (currentUserUid) window.syncToCloud(); }
    if(state.isBoss) { showBossToast(bInfo.name); spawnMonster(); }
    updateUI();
}

function showUpgradeToast(idChar, amt) {
    let box = document.getElementById(`upg-${idChar}-box`); let floatEl = document.createElement('div'); floatEl.className = 'upgrade-toast'; floatEl.innerText = '+' + amt;
...
window.upgrade = (type) => {
    ...
    if(state.mp >= u.cost) {
        state.mp -= u.cost; let amt = Math.floor(Math.random() * 6) + 1; u.val += amt; u.cost += Math.floor(u.cost * 0.2) + 3;
        let idChar = type === '전사' ? 'w' : (type === '법사' ? 'm' : 't'); showUpgradeToast(idChar, amt);
        document.getElementById(`upg-${idChar}-val`).innerText = u.val; document.getElementById(`upg-${idChar}-cost`).innerText = u.cost; updateUI();
...
window.openTicketModal = () => {
    ...
    currentTicketTier = state.tickets[0]; document.getElementById('ticket-tier').innerText = currentTicketTier; document.getElementById('overlay').style.display = 'block'; document.getElementById('ticket-modal').style.display = 'block'; updateUI();
};
window.useTicket = (choice) => {
    let emptyIdx = grid.findIndex(v => v === null); if(emptyIdx === -1) { showMessage("공간 부족!"); return; }
    state.tickets.shift(); let tier = currentTicketTier; let cls = choice === '랜덤' ? Object.keys(CLASSES)[Math.floor(Math.random()*3)] : choice; if(choice === '랜덤' && Math.random() < 0.2) tier++;
    window.addUnit(emptyIdx, tier, cls); window.closeAllModals(); updateUI();
};

window.toggleSpeed = () => {
    if(state.isRank) return;
    if (state.speed === 1) state.speed = 10; else if (state.speed === 10) state.speed = 15; else state.speed = 1; document.getElementById('btn-speed').innerText = state.speed + "배속";
};
...
function processOpponentTick(dt) {
    ...
    if(oppMonsters.length >= 50) { oppState.isDead = true; state.status = 'GAMEOVER'; processRankResult('WIN', '상대방의 몹이 50마리 쌓여 패배했습니다!'); return; }
    ...
    oppTowers.forEach(t => {
        ...
        if(t.lastAttack <= 0) {
            ...
            if(target) {
                let dmg = t.cls.baseDmg * t.grade.mult * cardMulti * rageMulti; let isCrit = Math.random() < sharpChance; if (isCrit) dmg *= 1.2;
...
function loop() {
    ...
    let now = performance.now(); let dt = ((now - lastTime) / 1000) * state.speed; if (dt > 0.1) dt = 0.1; lastTime = now;
    ...
    if (state.status === 'PREP') {
        state.time -= dt; document.getElementById('ui-timer').innerText = Math.ceil(state.time);
        if (state.time <= 0) { state.status = 'PLAY'; state.wave = state.wave || 1; waveTimer = 0; spawnTimer = 0; showMessage(state.wave + "웨이브 시작!"); updateUI(); }
        draw(); if(state.isRank) drawOpp(); mainReqId = requestAnimationFrame(loop); return;
    }
        ...
        if (t.gradeIdx >= 5) {
            if ((t.cls.type === '전사' && skillLevels.war_death > 0) || (t.cls.type === '법사' && skillLevels.mage_thunder > 0) || (t.cls.type === '도적' && skillLevels.thief_fuma > 0)) {
                ...
                if (t.globalCooldown <= 0 && monsters.length > 0) {
                    ...
                    if (t.cls.type === '전사' && skillLevels.war_death > 0) { let gdmg = baseDmg * (1 + skillLevels.war_death * 0.1); visualEffects.push({ type: 'death', timer: 1.2, dmg: gdmg }); t.globalCooldown = 60000; if(state.isRank && state.isBoss) rankState.myBossDamage += gdmg; }
                    else if (t.cls.type === '법사' && skillLevels.mage_thunder > 0) { let gdmg = baseDmg * (1 + skillLevels.mage_thunder * 0.1); visualEffects.push({ type: 'thunder', timer: 0.5, dmg: gdmg }); t.globalCooldown = 60000; if(state.isRank && state.isBoss) rankState.myBossDamage += gdmg; }
                    else if (t.cls.type === '도적' && skillLevels.thief_fuma > 0) { let gdmg = baseDmg * (1 + skillLevels.thief_fuma * 0.1); fumaList.push({ x: t.x, y: t.y, targetNode: 0, nodesVisited: 0, dmg: gdmg, hitSet: new Set(), angle: 0 }); t.globalCooldown = 60000; }
                }
            }
        }
        t.lastAttack -= dt * 1000;
    ...
    for(let i=monsters.length-1; i>=0; i--) {
        if(monsters[i].hp <= 0) {
            ...
            monsters.splice(i, 1); updateUI();
        }
    }
    
    draw(); if(state.isRank) drawOpp(); document.getElementById('ui-mobs').innerText = `${monsters.length} / 50`; mainReqId = requestAnimationFrame(loop);
}

function drawOpp() {
    if(!oppCtx) return; oppCtx.clearRect(0, 0, oppCanvas.width, oppCanvas.height); oppCtx.setLineDash([]); oppCtx.strokeStyle = "rgba(188, 170, 164, 0.2)"; oppCtx.lineWidth = 35; oppCtx.lineJoin = "round"; oppCtx.beginPath(); oppCtx.rect(25, 25, 450, 240); oppCtx.stroke();
    ...
    oppFumaList.forEach(f => { oppCtx.save(); oppCtx.translate(f.x, f.y); oppCtx.rotate(f.angle); if (fumaImg && fumaImg.complete) { let size = 60; oppCtx.drawImage(fumaImg, -size/2, -size/2, size, size); } oppCtx.restore(); });
    oppMonsters.forEach(m => {
        let size = m.isBoss ? 16 : 10; 
        if (m.isBoss && bossImages[m.name] && bossImages[m.name].complete) { oppCtx.save(); oppCtx.translate(m.x, m.y); if (m.facingRight) oppCtx.scale(-1, 1); if (m.freezeTimer > 0) { oppCtx.globalAlpha = 0.5; oppCtx.fillStyle = "#81d4fa"; oppCtx.fillRect(-size * 1.5, -size * 1.5, size * 3, size * 3); oppCtx.globalAlpha = 1.0; } oppCtx.drawImage(bossImages[m.name], -size * 1.5, -size * 1.5, size * 3, size * 3); oppCtx.restore(); } 
        else { if (!m.isBoss) { oppCtx.fillStyle = m.freezeTimer > 0 ? "#81d4fa" : "#81c784"; oppCtx.beginPath(); oppCtx.arc(m.x, m.y + 2, size, Math.PI, 0); oppCtx.fillRect(m.x - size, m.y + 2, size*2, size/2); oppCtx.fill(); } else { oppCtx.fillStyle = m.freezeTimer > 0 ? "#81d4fa" : "#ff8a65"; oppCtx.beginPath(); oppCtx.arc(m.x, m.y - 2, size, Math.PI, 0); oppCtx.fill(); oppCtx.fillStyle = m.freezeTimer > 0 ? "#b3e5fc" : "#ffe0b2"; oppCtx.fillRect(m.x - size/2, m.y - 2, size, size - 2); } }
        oppCtx.fillStyle = "#000"; oppCtx.fillRect(m.x-10, m.y-size-8, 20, 3); oppCtx.fillStyle = "#4caf50"; oppCtx.fillRect(m.x-10, m.y-size-8, 20 * (m.hp/m.maxHp), 3);
    });
    oppProjectiles.forEach(p => {
        oppCtx.save(); oppCtx.translate(p.x, p.y); let dir = Math.atan2(p.ty - p.y, p.tx - p.x); let scale = p.gradeIdx >= 6 ? 1.5 : 1; if (p.isFinal) scale *= 1.3; oppCtx.scale(scale, scale); if (p.isShadow) oppCtx.globalAlpha = 0.5;
        let img = null; let size = p.gradeIdx >= 5 ? 35 : 20;
        if (p.type === '전사') { img = p.gradeIdx >= 5 ? projImages.warrior2 : projImages.warrior1; oppCtx.rotate(dir + Math.PI); } 
        else if (p.type === '법사') { img = p.gradeIdx >= 5 ? projImages.mage2 : projImages.mage1; let mageAngle = p.gradeIdx >= 5 ? dir + (15 * Math.PI / 180) : dir; oppCtx.rotate(mageAngle); } 
        else if (p.type === '도적') { img = p.gradeIdx >= 5 ? projImages.rogue2 : projImages.rogue1; if (p.gradeIdx >= 5) size = 25; oppCtx.rotate(p.angle); }
        if (img && img.complete) { oppCtx.drawImage(img, -size/2, -size/2, size, size); } oppCtx.restore();
    });
    oppDamageTexts.forEach(d => { oppCtx.save(); oppCtx.globalAlpha = d.timer / 0.8; oppCtx.fillStyle = "#ffeb3b"; oppCtx.font = "bold 16px NanumSquare"; oppCtx.shadowColor = "#c62828"; oppCtx.shadowBlur = 4; oppCtx.fillText(d.val, d.x - 10, d.y); oppCtx.restore(); });
}

function draw() {
    if (!ctx) return; ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = "rgba(188, 170, 164, 0.2)"; ctx.lineWidth = 35; ctx.lineJoin = "round"; ctx.beginPath(); if (state.isRank) { ctx.rect(25, 25, 450, 240); } else { ctx.rect(25, 25, 450, 450); } ctx.stroke();
    visualEffects.forEach(v => {
        ctx.save();
        if (v.type === 'death') { let progress = Math.min(1, (1.2 - v.timer) / 0.2); ctx.strokeStyle = "#ffeb3b"; ctx.lineWidth = 8; ctx.lineCap = "round"; ctx.shadowColor = "#f57f17"; ctx.shadowBlur = 10; let currentX = -50 + (600) * progress; let currentY = 450 + (-400) * progress; ctx.beginPath(); ctx.moveTo(-50, 450); ctx.lineTo(currentX, currentY); ctx.stroke(); } 
        else if (v.type === 'thunder') { ctx.fillStyle = `rgba(0, 229, 255, ${v.timer})`; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = `rgba(255, 255, 255, ${v.timer * 2})`; ctx.lineWidth = 15; ctx.beginPath(); ctx.moveTo(canvas.width / 2, 0); ctx.lineTo(canvas.width / 2 - 50, canvas.height / 2); ctx.lineTo(canvas.width / 2 + 50, canvas.height / 2); ctx.lineTo(canvas.width / 2, canvas.height); ctx.stroke(); }
        ctx.restore();
    });
    hitEffects.forEach(h => { ctx.save(); ctx.globalAlpha = Math.max(0, h.timer / 0.2); ctx.fillStyle = h.color || "#fff"; ctx.shadowColor = h.color || "#fff"; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(h.x, h.y, 15 - (h.timer * 30), 0, Math.PI * 2); ctx.fill(); ctx.restore(); });
    fumaList.forEach(f => { ctx.save(); ctx.translate(f.x, f.y); ctx.rotate(f.angle); if (fumaImg && fumaImg.complete) { let size = 60; ctx.drawImage(fumaImg, -size/2, -size/2, size, size); } ctx.restore(); });
    monsters.forEach(m => {
        let size = m.isBoss ? 16 : 10; 
        if (m.isBoss && bossImages[m.name] && bossImages[m.name].complete) { ctx.save(); ctx.translate(m.x, m.y); if (m.facingRight) ctx.scale(-1, 1); if (m.freezeTimer > 0) { ctx.globalAlpha = 0.5; ctx.fillStyle = "#81d4fa"; ctx.fillRect(-size * 1.5, -size * 1.5, size * 3, size * 3); ctx.globalAlpha = 1.0; } if (m.bindTimer > 0) { ctx.globalAlpha = 0.5; ctx.fillStyle = "#00e5ff"; ctx.fillRect(-size * 1.5, -size * 1.5, size * 3, size * 3); ctx.globalAlpha = 1.0; } ctx.drawImage(bossImages[m.name], -size * 1.5, -size * 1.5, size * 3, size * 3); ctx.restore(); } 
        else { if (!m.isBoss) { ctx.fillStyle = m.freezeTimer > 0 ? "#81d4fa" : (m.bindTimer > 0 ? "#00e5ff" : "#81c784"); ctx.beginPath(); ctx.arc(m.x, m.y + 2, size, Math.PI, 0); ctx.fillRect(m.x - size, m.y + 2, size*2, size/2); ctx.fill(); } else { ctx.fillStyle = m.freezeTimer > 0 ? "#81d4fa" : (m.bindTimer > 0 ? "#00e5ff" : "#ff8a65"); ctx.beginPath(); ctx.arc(m.x, m.y - 2, size, Math.PI, 0); ctx.fill(); ctx.fillStyle = m.freezeTimer > 0 ? "#b3e5fc" : (m.bindTimer > 0 ? "#84ffff" : "#ffe0b2"); ctx.fillRect(m.x - size/2, m.y - 2, size, size - 2); } }
        if (m.bindTimer > 0) { ctx.fillStyle = "#00e5ff"; ctx.font = "bold 12px sans-serif"; ctx.fillText("Bind!", m.x - 15, m.y - size - 15); } else if (m.stunTimer > 0) { ctx.fillStyle = "#ffeb3b"; ctx.font = "bold 12px sans-serif"; ctx.fillText("Stun!", m.x - 15, m.y - size - 15); }
        ctx.fillStyle = "#000"; ctx.fillRect(m.x-10, m.y-size-8, 20, 3); ctx.fillStyle = "#4caf50"; ctx.fillRect(m.x-10, m.y-size-8, 20 * (m.hp/m.maxHp), 3);
    });
    projectiles.forEach(p => {
        ctx.save(); ctx.translate(p.x, p.y); let dir = Math.atan2(p.ty - p.y, p.tx - p.x); let scale = p.gradeIdx >= 6 ? 1.5 : 1; if (p.isFinal) scale *= 1.3; ctx.scale(scale, scale); if (p.isShadow) ctx.globalAlpha = 0.5;
        let img = null; let size = p.gradeIdx >= 5 ? 35 : 20; 
        if (p.type === '전사') { img = p.gradeIdx >= 5 ? projImages.warrior2 : projImages.warrior1; ctx.rotate(dir + Math.PI); } 
        else if (p.type === '법사') { img = p.gradeIdx >= 5 ? projImages.mage2 : projImages.mage1; let mageAngle = p.gradeIdx >= 5 ? dir + (15 * Math.PI / 180) : dir; ctx.rotate(mageAngle); } 
        else if (p.type === '도적') { img = p.gradeIdx >= 5 ? projImages.rogue2 : projImages.rogue1; if (p.gradeIdx >= 5) size = 25; ctx.rotate(p.angle); }
        if (img && img.complete) { ctx.drawImage(img, -size/2, -size/2, size, size); } ctx.restore();
    });
    damageTexts.forEach(d => { ctx.save(); ctx.globalAlpha = Math.max(0, d.timer / 0.8); ctx.fillStyle = "#ffeb3b"; ctx.font = "bold 16px sans-serif"; ctx.shadowColor = "#c62828"; ctx.shadowBlur = 4; ctx.fillText(d.val, d.x - 10, d.y); ctx.restore(); });
    if (selectedUnitIdx !== -1 && grid[selectedUnitIdx]) { let u = grid[selectedUnitIdx]; let currentRange = u.cls.range * u.grade.rangeMul; ctx.save(); ctx.beginPath(); ctx.arc(u.x, u.y, currentRange, 0, Math.PI * 2); ctx.fillStyle = "rgba(255, 255, 255, 0.15)"; ctx.fill(); ctx.strokeStyle = "rgba(255, 255, 255, 0.6)"; ctx.lineWidth = 1.5; ctx.setLineDash([5, 5]); ctx.stroke(); ctx.setLineDash([]); ctx.restore(); }
}
...
function showMessage(msg) { let ov = document.getElementById('msg-overlay'); ov.innerText = msg; ov.style.display = 'block'; setTimeout(() => { ov.style.display = 'none'; }, 2000); }
...
window.showPkClassSelect = () => { if (!currentUserUid) return alert("로그인이 필요한 서비스입니다."); document.getElementById('pk-menu').style.display = 'none'; document.getElementById('pk-class-select').style.display = 'block'; };
...
function pkLoop() { 
    ...
    let now = performance.now(); let dt = ((now - pkState.lastTime) / 1000) * pkState.speed; if (dt > 0.1) dt = 0.1; pkState.lastTime = now;
    ...
    if ((u.cls.type === '전사' && skillLevels.war_death > 0) || (u.cls.type === '법사' && skillLevels.mage_thunder > 0) || (u.cls.type === '도적' && skillLevels.thief_fuma > 0)) {
        ...
        if (u.globalCooldown <= 0) {
            ...
            if (u.cls.type === '전사' && skillLevels.war_death > 0) { let gdmg = baseDmg * (1 + skillLevels.war_death * 0.1); pkState.vfx.push({ type: 'death', timer: 1.2, dmg: gdmg }); u.globalCooldown = 60000; } 
            else if (u.cls.type === '법사' && skillLevels.mage_thunder > 0) { let gdmg = baseDmg * (1 + skillLevels.mage_thunder * 0.1); pkState.vfx.push({ type: 'thunder', timer: 0.5, dmg: gdmg }); u.globalCooldown = 60000; } 
            else if (u.cls.type === '도적' && skillLevels.thief_fuma > 0) { let gdmg = baseDmg * (1 + skillLevels.thief_fuma * 0.1); pkApplyDmg(gdmg, false); pkState.vfx.push({ type: 'fuma', timer: 0.5 }); u.globalCooldown = 60000; }
        }
    }
...
function drawPk() {
    ...
    pkState.vfx.forEach(v => { if (v.type === 'fuma') { pkCtx.save(); pkCtx.translate(m.x, m.y); pkCtx.rotate((0.5 - v.timer) * 30); if (fumaImg && fumaImg.complete) { let fsize = 80; pkCtx.drawImage(fumaImg, -fsize/2, -fsize/2, fsize, fsize); } pkCtx.restore(); } });
...
function showMatchIntro(oppName, oppRp, callback) {
    ...
    intro.style.display = 'flex'; void intro.offsetWidth; intro.style.opacity = '1';
    setTimeout(() => { intro.style.opacity = '0'; setTimeout(() => { intro.style.display = 'none'; callback(); }, 500); }, 2000); 
}
window.startRankMatchmaking = async () => {
    let today = new Date().toLocaleDateString(); let playCount = parseInt(localStorage.getItem('mapleDefenseRankCount')) || 0;
    if (playCount >= 10) { alert("오늘의 랭크 게임 제한 횟수(10회)를 모두 소진했습니다. 내일 다시 도전해주세요!"); return; }
...
function enterRankGameAI(oppName, oppRp) {
    ...
    state = { status: 'PREP', meso: 100, mp: 0, mpTotal: 0, kills: 0, wave: 1, time: 5, speed: 30, isBoss: false, upgrades: { '전사': {val: 0, cost: 10}, '법사': {val: 0, cost: 10}, '도적': {val: 0, cost: 10} }, tickets: [], isRank: true };
    ...
    renderGrid(); window.switchScreen('game-container'); document.getElementById('btn-speed').style.display = 'none'; document.getElementById('btn-exit').style.display = 'none'; let surrenderBtn = document.getElementById('btn-rank-surrender'); if (surrenderBtn) surrenderBtn.style.display = 'block';
    document.getElementById('opp-board-wrapper').style.display = 'flex'; document.getElementById('opp-name').innerText = oppName; document.getElementById('opp-wave').innerText = '1'; document.getElementById('opp-mobs').innerText = '0';
...
window.renderInventoryTab = (tab) => {
    ...
    if (tab === 'consumable') {
        ...
        ['브론즈', '실버', '골드', '플래티넘', '다이아몬드', '챌린저'].forEach(tier => {
            if (userInventory.boxes[tier] > 0) { list.innerHTML += createInvBox('🧰', `${tier} 상자`, userInventory.boxes[tier], `openBox('${tier}')`); }
        });
    } else if (tab === 'equip') {
...
function createInvBox(icon, name, qty, onclickStr) {
    return `<div class="inv-item-box" onclick="${onclickStr}">
        <div class="inv-item-icon">${icon}</div>
        <div class="inv-item-qty">${qty}</div>
        <div style="font-size:10px; font-weight:bold; color:#546e7a; margin-top:2px; text-align:center;">${name}</div>
    </div>`;
}

window.openBox = (boxType) => {
    let tierProb = {
        '브론즈': { frag: [ [1, 0.6], [0, 0.4] ], equip: 0 },
        '실버': { frag: [ [1, 0.666], [2, 0.333] ], equip: 0 },
        '골드': { frag: [ [2, 0.45], [3, 0.35], [4, 0.15], [5, 0.05] ], equip: 0.05 },
        '플래티넘': { frag: [ [3, 0.55], [4, 0.35], [5, 0.1] ], equip: 0.15 },
        '다이아몬드': { frag: [ [4, 0.666], [5, 0.333] ], equip: 0.35 },
        '챌린저': { frag: [ [5, 1.0] ], equip: 0.55 }
    };
    ...
    if (boxType === 'equipBoxes') {
        ...
        userInventory.coinPieces += getFrag;
        
        let getEquipBox = Math.random() < data.equip ? 1 : 0;
        userInventory.equipBoxes += getEquipBox;
        alert(`${boxType} 상자 개봉!\n코인 조각: +${getFrag}\n장비 상자: +${getEquipBox}`);
    }
...
function generateEquipment() {
    let types = ['뱃지', '엠블럼', '링']; let type = types[Math.floor(Math.random() * 3)];
    let r = Math.random(); let grade, min, max;
    ...
    let atk = 0, spd = 0, crit = 0;
    let statType = Math.floor(Math.random() * 3);
    let val = Math.floor(Math.random() * (max - min + 1)) + min;
    
    let alertMsg = "";
    if (statType === 0) { atk = val; alertMsg = `공격력: +${val}%`; }
    else if (statType === 1) { spd = val; alertMsg = `공격속도: +${val}%`; }
    else { crit = val; alertMsg = `크리티컬: +${val}%`; }

    let newEquip = { type, grade, atk, spd, crit, id: Date.now() };
    userEquips.push(newEquip);
    alert(`[${grade}] ${type} 획득!\n${alertMsg}`);
}
...
window.unequipItem = (type) => {
    if(userEquipped[type]) {
        userEquips.push(userEquipped[type]); userEquipped[type] = null;
        calculateEquipStats(); window.syncToCloud(); renderEquippedSlots(); renderInventoryTab('equip');
    }
};
...
window.openRaidMenu = async () => {
    ...
    if (lastRaidDate === today) { alert('오늘 이미 월드 보스 토벌에 참여하셨습니다. 내일 다시 도전해 주세요!'); return; }
    if (!currentUserUid) return alert("로그인이 필요한 서비스입니다.");
    ...
    raidState.initialKillCount = -1; raidState.rewardClaimedForKills = [];

    document.getElementById('raid-prep-ui').style.display = 'flex';
    document.getElementById('raid-prep-time').innerText = '10';
    document.getElementById('raid-meso').innerText = '30';
    renderRaidGrid();
    ...
    onValue(ref(database, 'worldBoss/killCount'), (snap) => {
        if(snap.exists()) {
            let currentKills = snap.val();
            if (raidState.initialKillCount === -1) {
                raidState.initialKillCount = currentKills;
            } else if (raidState.active && currentKills > raidState.initialKillCount && raidState.totalDmg > 0) {
                if (!raidState.rewardClaimedForKills.includes(currentKills)) {
                    userInventory.boxes['골드'] = (userInventory.boxes['골드'] || 0) + 1;
                    window.syncToCloud();
                    let toast = document.getElementById('boss-toast');
                    toast.innerHTML = "🎉 월드 보스 토벌 성공! 골드 보상 상자 획득! 🎉";
                    toast.style.color = "#ffd700"; toast.className = 'toast-show';
                    setTimeout(() => { toast.className = ''; }, 3000);
                    raidState.rewardClaimedForKills.push(currentKills);
                }
            }
        }
    });
...
window.summonRaidUnit = () => {
    if(raidState.status !== 'PREP') return;
    if(raidState.meso < 10) { alert("메소가 부족합니다."); return; }
    let emptyIdx = raidState.units.findIndex(v => v === null);
    if(emptyIdx === -1) { alert("더 이상 배치할 수 없습니다."); return; }
    
    raidState.meso -= 10;
    document.getElementById('raid-meso').innerText = raidState.meso;
    
    let r = Math.random() * 100;
    let gradeIdx = r < 60 ? 5 : (r < 90 ? 6 : (r < 99 ? 7 : 8)); 
    ...
    raidState.units[emptyIdx] = {
        cls: cls, grade: grade, gradeIdx: gradeIdx,
        x: 100 + (emptyIdx * 150), y: 400, lastAttack: 0, globalCooldown: 0
    };
    renderRaidGrid();
};
...
function renderRaidGrid() {
    ...
    for(let i=0; i<3; i++) {
        ...
        if (u) {
            gridHtml += `<div class="grid-cell glow-${u.gradeIdx}" style="display:flex;flex-direction:column;align-items:center;justify-content:center;">
                <div style="font-size:24px;">${u.cls.icon}</div>
                <div style="font-size:10px; color:${u.cls.color}; font-weight:bold;">${u.grade.name}</div>
            </div>`;
        } else {
            gridHtml += `<div class="grid-cell" style="background:rgba(0,0,0,0.2); border:1px dashed #777;"></div>`;
        }
    }
...
function raidLoop() {
    ...
    if (raidState.status === 'PREP') {
        raidState.prepTime -= dtReal;
        document.getElementById('raid-prep-time').innerText = Math.ceil(Math.max(0, raidState.prepTime));
        if (raidState.prepTime <= 0) {
            raidState.status = 'PLAY';
            document.getElementById('raid-prep-ui').style.display = 'none';
        }
        drawRaid();
        raidReqId = requestAnimationFrame(raidLoop);
        return;
    }
    ...
    raidState.units.forEach(u => {
        ...
        if (u.lastAttack <= 0) {
            ...
            raidState.projectiles.push({ type: u.cls.type, x: u.x, y: u.y, tx: bx, ty: by, dmg: dmg, color: u.cls.color, angle: 0, isCrit: isCrit, gradeIdx: u.gradeIdx });
    ...
    for (let i = raidState.projectiles.length - 1; i >= 0; i--) {
        ...
        if (dist <= speed) {
            ...
            raidState.dmgTexts.push({ val: Math.floor(p.dmg), x: bx + ox, y: by + oy, timer: 0.6, isCrit: p.isCrit });
...
function drawRaid() {
    ...
    raidState.projectiles.forEach(p => {
        ctx.save(); ctx.translate(p.x, p.y); ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(0, 0, p.gradeIdx >= 6 ? 8 : 5, 0, Math.PI*2); ctx.fill(); ctx.restore();
    });
    raidState.dmgTexts.forEach(d => {
        ctx.save(); ctx.globalAlpha = Math.max(0, d.timer / 0.6); ctx.fillStyle = d.isCrit ? "#ffeb3b" : "#fff"; ctx.font = d.isCrit ? "900 24px NanumSquare" : "bold 18px NanumSquare"; ctx.shadowColor = d.isCrit ? "#c62828" : "#000"; ctx.shadowBlur = 4; ctx.fillText(d.val, d.x - 20, d.y); ctx.restore();
    });
}
...
function endRaidGame() {
    ...
    userInventory.boxes[rewardTier]++; window.syncToCloud();
    document.getElementById('raid-result-dmg').innerText = Math.floor(raidState.totalDmg).toLocaleString(); document.getElementById('raid-result-percent').innerText = percent.toFixed(2) + "%"; document.getElementById('raid-result-rewards').innerText = `🎁 ${rewardTier} 상자 1개 지급 완료!`;

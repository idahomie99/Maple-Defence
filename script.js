// 🔥 1.0.40 버전 - 중복 코드 대청소, 펀치킹 딜레이 완벽 제거, Alert 알림 교체
...
function setGridMode(mode) {
    ...
    canvas.height = canvasHeight; document.getElementById('board-area').style.aspectRatio = isRankGrid ? "500/290" : "1/1";
...
function showUpgradeToast(idChar, amt) {
    let box = document.getElementById(`upg-${idChar}-box`); let floatEl = document.createElement('div'); floatEl.className = 'upgrade-toast'; floatEl.innerText = '+' + amt; box.appendChild(floatEl); setTimeout(() => floatEl.remove(), 1000);
}
...
window.renderBook = () => { /* ... (기존과 동일하여 축약 없이 유지) ... */
...
// 🔥 펀치킹 시작 시 즉각적으로 로딩되도록 비동기 대기(await) 제거
